...
def mlmc_test(no_parallel_processes, path_stem, mlmc_fn, calc_formula, wavetype, dictionary_type, M, L, N0, Lmin, Lmax, logfile, build_output, variable_name, sigma_function, interp_fn = 0, angles_fn = 0):
    """
    ...
    for l in range(Lmin, L+1):
        ...
        sums = mlmc_fn(processes = no_parallel_processes, path_stem = path_stem, calc_formula = calc_formula, l = l, M = M, build_output = build_output, variable_name = variable_name, sigma_function = sigma_function, interp_fn = interp_fn, angles_fn = angles_fn, Lmin = Lmin, Lmax = Lmax, wavetype = wavetype, dictionary_type = dictionary_type, iteration = N0)
...
def eps_only(alpha, beta, gamma, Vl_orig, ml_orig, no_parallel_processes, path_stem, mlmc_fn, calc_formula, wavetype, dictionary_type, M, L, min_num, Eps, Lmin, Lmax, logfile, build_output, variable_name, sigma_function, interp_fn = 0, angles_fn = 0):
            ...
            if first_pass == False:
                ...
                for l in range(0, L+1-Lmin):
                    if dNl[l] > 0:
                        ...
                        sums = mlmc_fn(no_parallel_processes, path_stem, calc_formula, l+Lmin, M, build_output, variable_name, sigma_function, interp_fn, angles_fn, Lmin, Lmax, wavetype = wavetype, dictionary_type = dictionary_type, iteration = int(dNl[l]))
                        kurt = (     sums[3]
                                - 4*sums[2]*sums[0]
                                + 6*sums[1]*sums[0]**2
                                - 3*sums[0]*sums[0]**3 ) / (sums[1]-sums[0]**2)**2
